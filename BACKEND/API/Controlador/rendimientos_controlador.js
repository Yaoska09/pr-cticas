...
const consultas = require('../Modelo/RENDIMIENTO/rendimientos_consultas');
...
const add = (req, res) => {
    const { id_menor,id_proyecto,nota } = req.body;
    pool.query(consultas.getByCorreo, [correo], (error, results) => {//modificar siguiendo el archivo de consulta en  la carpeta MODELO
        ...
        pool.query(consultas.add, [id_menor,id_proyecto,nota ], (error, results) => {
            if (error) throw error;
...
const getById_menor = (req, res) => {
    ...
    pool.query(consultas.getByCorreo, [id_menor], (error, results) => {
        if (error) throw error;
        res.status(200).json(results.rows);
    });
};

const getById_proyecto = (req, res) => {
    const correo = req.params.id_proyecto;
    pool.query(consultas.getByCorreo, [id_proyecto], (error, results) => {
        if (error) throw error;
        res.status(200).json(results.rows);
    });
};
